...
@implementation webcapsule_mobileObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_webcapsule_mobile);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any webcapsule_mobile patches in JS before a context is loaded... nice for 
...
{
	// create webcapsule_mobile global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_webcapsule_mobile = [[webcapsule_mobileObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_webcapsule_mobile];
	
	NSString *_webcapsule_mobileNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_webcapsule_mobileNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_webcapsule_mobile valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.webcapsule_mobile.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
